...
public:
    CScriptCheck() : ptxTo(0), nIn(0), nFlags(0), cacheStore(false) /* , error(SCRIPT_ERR_UNKNOWN_ERROR) */ {} // <<< REMOVE error(...) PART
    CScriptCheck(const CCoins& txFromIn, const CTransaction& txToIn, unsigned int nInIn, unsigned int nFlagsIn, bool cacheIn) :
        scriptPubKey(txFromIn.vout[txToIn.vin[nInIn].prevout.n].scriptPubKey),
        ptxTo(&txToIn), nIn(nInIn), nFlags(nFlagsIn), cacheStore(cacheIn) /* , error(SCRIPT_ERR_UNKNOWN_ERROR) */ {} // <<< REMOVE error(...) PART
    ...
    void swap(CScriptCheck& check)
    {
        ...
        // std::swap(error, check.error); // <<< REMOVE THIS LINE
    }
