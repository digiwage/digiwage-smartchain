...
// In class CTxOut (DigiWage's primitives/transaction.h):
bool IsDust(CFeeRate minRelayTxFee) const
{
    // If it's a contract creation or call, it's never dust, even if 0-value.
    // This assumes you have ported/implemented HasOpCreate() and HasOpCall() in CScript
    if (scriptPubKey.HasOpCreate() || scriptPubKey.HasOpCall()) { // <<< QTUM-LIKE MODIFICATION
        return false;
    }

    // Original dust logic for non-contract outputs:
