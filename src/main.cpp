...
// Copyright (c) 2017-TBD The Qtum/DigiWage EVM developers
...
#include "script/standard.h"
#include "hash.h"
#include <libdevcore/OverlayDB.h> // This might define CacheState
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // Log info about the invalid block found (pindexNew should be valid here)
    ...
    // ---> ADD NULL CHECK FOR CHAIN TIP <---
    const CBlockIndex* pChainTip = chainActive.Tip();
    if (pChainTip) {
        // Log info about the current best chain *if* the tip exists
        LogPrintf("InvalidChainFound:  current best=%s  height=%d  log2_work=%.16f  date=%s\n",
                  pChainTip->GetBlockHash().GetHex(), // Use pChainTip directly
                  pChainTip->nHeight,
                  log(pChainTip->nChainWork.getdouble()) / log(2.0),
                  DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pChainTip->GetBlockTime()));
    } else {
        // Log that the chain tip is currently null (might happen during initial errors)
        LogPrintf("InvalidChainFound: Current chain tip is NULL.\n");
    }
...
// --- CheckBlock (Modified for EVM with Corrected Error Handling) ---
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) {
    ...
    if (!CheckBlockHeader(block, state, !IsPoS)) { // !IsPoS is true for PoW blocks
        // CheckBlockHeader already called state.DoS and error if it failed.
        // No need to call state.DoS() again or error() again if CheckBlockHeader handles it.
        // We just need to ensure CheckBlock propagates the failure.
        // Add a general log here if specific logging in CheckBlockHeader isn't verbose enough.
        // LogPrintf("%s : CheckBlockHeader failed for block %s\n", __func__, block.GetHash().ToString());
    ...
    if (fCheckMerkleRoot) {
        ...
        if (block.hashMerkleRoot != hashMerkleRoot2) {
            error("%s : hashMerkleRoot mismatch for block %s", __func__, block.GetHash().ToString());
            return state.DoS(100, false, REJECT_INVALID, "bad-txnmrklroot", true);
        }
        if (mutated) {
            error("%s : duplicate transaction in block %s", __func__, block.GetHash().ToString());
    ...
    if (block.vtx.empty() || block.vtx.size() > nMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > nMaxBlockSize) {
        error("%s : size limits failed for block %s", __func__, block.GetHash().ToString());
        return state.DoS(100, false, REJECT_INVALID, "bad-blk-length");
    }

    // First transaction must be coinbase
    if (block.vtx.empty() || !block.vtx[0].IsCoinBase()) {
        error("%s : first tx is not coinbase in block %s", __func__, block.GetHash().ToString());
        return state.DoS(100, false, REJECT_INVALID, "bad-cb-missing");
    }
    for (unsigned int i = 1; i < block.vtx.size(); i++) {
        if (block.vtx[i].IsCoinBase()) {
            error("%s : more than one coinbase in block %s", __func__, block.GetHash().ToString());
    ...
    if(TxHasOpSpend(block.vtx[0]) || TxHasCreateOrCall(block.vtx[0]) || TxHasOpSender(block.vtx[0])){
        error("%s: coinbase contains contract opcodes in block %s", __func__, block.GetHash().ToString());
        return state.DoS(100, false, REJECT_INVALID, "bad-cb-contract");
    }

    if (IsPoS) {
        // Coinbase output should be empty if proof-of-stake block
        if (block.vtx[0].vout.size() != 1 || !block.vtx[0].vout[0].IsEmpty()) {
            error("%s : coinbase output not empty for proof-of-stake block %s", __func__, block.GetHash().ToString());
            return state.DoS(100, false, REJECT_INVALID, "bad-cb-pos-notempty"); // Specific reason
        }
        // Second transaction must be coinstake
        if (block.vtx.empty() || block.vtx.size() < 2 || !block.vtx[1].IsCoinStake()) {
            error("%s : second tx is not coinstake in PoS block %s", __func__, block.GetHash().ToString());
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing");
        }
        for (unsigned int i = 2; i < block.vtx.size(); i++) {
            if (block.vtx[i].IsCoinStake()) {
                error("%s : more than one coinstake in block %s", __func__, block.GetHash().ToString());
                return state.DoS(100, false, REJECT_INVALID, "bad-cs-multiple");
            }
        }
        ...
        if(TxHasOpSpend(block.vtx[1]) || TxHasCreateOrCall(block.vtx[1]) || TxHasOpSender(block.vtx[1])){
            error("%s: coinstake contains contract opcodes in block %s", __func__, block.GetHash().ToString());
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-contract");
        }
    }
                ...
                for (const CTxIn& in : tx.vin) {
                    if (mapLockedInputs.count(in.prevout)) {
                        if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                            ...
                            error("%s : found conflicting transaction with transaction lock in block %s", __func__, block.GetHash().ToString());
                            return state.DoS(0, false, REJECT_INVALID, "conflicting-tx-ix");
                        }
                    }
                }
            }
        }
    } else { LogPrintf("%s : skipping transaction locking checks for block %s\n", __func__, block.GetHash().ToString()); }

    // Cold Staking / Masternode / Budget checks
    bool fColdStakingActive = true;
    bool fZerocoinActive = block.GetBlockTime() > Params().GetConsensus().ZC_TimeStart; // Still from merged
    ...
    if (pindexPrev) {
        ...
        if (!IsInitialBlockDownload()) {
             if (IsPoS && nHeight > 0 && !CheckColdStakeFreeOutput(block.vtx[1], nHeight)) {
                 mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                 error("%s : Cold stake outputs not valid for block %s", __func__, block.GetHash().ToString());
                 return state.DoS(0, false, REJECT_INVALID, "bad-p2cs-outs");
             }
             if (nHeight > 0 && !IsBlockPayeeValid(block, nHeight)) {
                 mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                 error("%s : Couldn't find masternode/budget payment for block %s", __func__, block.GetHash().ToString());
                 return state.DoS(0, false, REJECT_INVALID, "bad-cb-payee");
             }
        } else { if (fDebug) LogPrintf("%s: Masternode/Budget/P2CS payment checks skipped on sync for block %s\n", __func__, block.GetHash().ToString()); }
    }
    ...
    for (const CTransaction& tx : block.vtx) {
        if (!CheckTransaction(tx, fZerocoinActive, state, fColdStakingActive)) {
            error("%s : CheckTransaction failed for tx %s in block %s", __func__, tx.GetHash().ToString(), block.GetHash().ToString());
            return false; // CheckTransaction should set state and return false
        }
        ...
        if(thisHasOpSpend && !lastWasContractOrSpend && !TxHasCreateOrCall(tx)){
            error("%s: OP_SPEND without preceding contract/spend tx %s in block %s", __func__, tx.GetHash().ToString(), block.GetHash().ToString());
            return state.DoS(100, false, REJECT_INVALID, "bad-opspend-tx");
        }
        lastWasContractOrSpend = TxHasCreateOrCall(tx) || thisHasOpSpend;
    }

    // Check legacy sigops
    unsigned int nSigOpsCountBlock = 0; // Use a different name
    for (const CTransaction& tx : block.vtx) { nSigOpsCountBlock += GetLegacySigOpCount(tx); }
    unsigned int nMaxBlockSigOps = MAX_BLOCK_SIGOPS_CURRENT;
    if (nSigOpsCountBlock > nMaxBlockSigOps) {
        error("%s : out-of-bounds SigOpCount for block %s (%u > %u)", __func__, block.GetHash().ToString(), nSigOpsCountBlock, nMaxBlockSigOps);
    ...
    if (IsPoS && fCheckSig && !CheckBlockSignature(block)) {
        LogPrintf("%s: bad PoS block signature for block %s\n", __func__, block.GetHash().ToString());
        return state.Invalid(false, REJECT_INVALID, "bad-blk-sig");
    }
...
// --- ContextualCheckBlockHeader (Keep DigiWage implementation, ADDING state root checks) ---
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev) {
    // --- Existing Code ---
    const Consensus::Params& consensus = Params().GetConsensus();
    uint256 hash = block.GetHash();
    if (hash == consensus.hashGenesisBlock) return true; // Genesis check remains

    assert(pindexPrev);
    const int nHeight = pindexPrev->nHeight + 1; // Calculate height of the block being checked

    const int chainHeight = chainActive.Height(); // Assuming chainActive is accessible or use pindexPrev for height checks relative to it
    int nMaxReorgDepth = GetArg("-maxreorg", DEFAULT_MAX_REORG_DEPTH);

    // Fork depth check (existing)
    if (chainHeight > nMaxReorgDepth && nHeight < chainHeight - nMaxReorgDepth){
         // Adjusted condition to avoid issues when chainHeight <= nMaxReorgDepth
         // Check if the block's height is older than the allowed reorg depth from the tip
         return state.DoS(1, error("%s: forked chain older than max reorganization depth (height %d)", __func__, nHeight), REJECT_INVALID, "bad-fork-prior-to-checkpoint"); // Use REJECT_INVALID? Check original code
    }

    // Time checks (existing)
    if (!CheckBlockTime(block, state, pindexPrev)) return false;

    // Checkpoint checks (existing)
    // Note: Ensure Checkpoints::CheckBlock and GetLastCheckpoint are correctly implemented and thread-safe if needed.
    ...
    // Fork older than last checkpoint check (existing)
    CBlockIndex* pcheckpoint = Checkpoints::GetLastCheckpoint();
    if (pcheckpoint && nHeight < pcheckpoint->nHeight) return state.DoS(0, error("%s : forked chain older than last checkpoint (height %d)", __func__, nHeight), REJECT_OBSOLETE, "bad-fork-prior-to-checkpoint"); // Use REJECT_OBSOLETE?

    // Version checks (existing) - Ensure these heights are correct for your chain
    if ((block.nVersion < 3 && nHeight >= 1) ||                          // Example heights, verify these
        (block.nVersion < 4 && nHeight >= consensus.height_start_ZC) ||  // Check if height_start_ZC is correct
        (block.nVersion < 5 && nHeight >= consensus.height_RHF)) {       // Check if height_RHF is correct
    ...
    // ---> ADD STATE ROOT PRESENCE/ABSENCE CHECKS HERE <---
    // Ensure consensus.height_start_StateRoots is defined in consensus/params.h
    // and set appropriately in chainparams.cpp for each network.
    if (nHeight >= consensus.height_start_StateRoots) {
        // After (or at) the state root fork height, roots MUST be present (non-null)
        // Assuming your CBlockHeader structure now includes hashStateRoot and hashUTXORoot
        if (block.hashStateRoot.IsNull() || block.hashUTXORoot.IsNull()) {
            // Reject block if roots are missing when they are required
            return state.DoS(100, error("%s: state roots missing in block header after activation height %d", __func__, consensus.height_start_StateRoots),
                             REJECT_INVALID, "bad-header-missing-stateroots");
        }
    } else {
        // Before the state root fork height, roots MUST be absent (null)
        if (!block.hashStateRoot.IsNull() || !block.hashUTXORoot.IsNull()) {
            // Reject block if roots are present before they are allowed
            return state.DoS(100, error("%s: premature state roots in block header before activation height %d", __func__, consensus.height_start_StateRoots),
                             REJECT_INVALID, "bad-header-premature-stateroots");
        }
    }
    // ---> END OF STATE ROOT CHECKS <---


    // EVM/PoS difficulty check moved comment (existing)
    // Note: The actual comparison of the state root hash values against calculated values
    // CANNOT happen here. It must happen later, likely in ConnectBlock or ContextualCheckBlock,
    // after transactions have been processed and state transitions calculated.
...
// Mega-Merged main.cpp
bool AcceptBlockHeader(const CBlockHeader& block_header_obj, CValidationState& state, CBlockIndex** ppindex) { // Renamed arg for clarity
    AssertLockHeld(cs_main);
    uint256 hash = block_header_obj.GetHash();
    ...
    if (miSelf != mapBlockIndex.end()) {
        ...
        if (pindex->nStatus & BLOCK_FAILED_MASK) {
            ...
            return state.Invalid(false, 0, "duplicate"); // Pass false to state.Invalid
        }
        return true;
    }
    ...
    if (hash != Params().GetConsensus().hashGenesisBlock) {
        auto mi = mapBlockIndex.find(block_header_obj.hashPrevBlock);
        if (mi == mapBlockIndex.end()) {
            error("%s : prev block %s not found for %s", __func__, block_header_obj.hashPrevBlock.GetHex(), hash.ToString());
            return state.DoS(0, false, 0, "bad-prevblk");
        }
        pindexPrev = (*mi).second;
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK) {
            if (Checkpoints::CheckBlock(pindexPrev->nHeight, block_header_obj.hashPrevBlock, true)) {
            ...
            error("%s : prev block height=%d hash=%s is invalid, unable to add block %s", __func__, pindexPrev->nHeight, block_header_obj.hashPrevBlock.GetHex(), hash.ToString());
            return state.DoS(100, false, REJECT_INVALID, "bad-prevblk");
        }
    }

    // Perform contextual checks on the header that *can* be done with only the header.
    // This includes timestamp, version vs height, checkpoints.
    // ContextualCheckBlockHeader should NOT perform a PoW hash check by itself.
    if (!ContextualCheckBlockHeader(block_header_obj, state, pindexPrev)) {
        // ContextualCheckBlockHeader should have set state and logged.
        return false;
    }

    // The strict PoW check (block.GetHash() <= target derived from block.nBits)
    // is deferred to CheckBlock(), which has the full CBlock object and can determine
    // if it's PoW or PoS before deciding to perform the PoW check.

    if (pindex == nullptr) pindex = AddToBlockIndex(block_header_obj); // Pass the CBlockHeader
    if (ppindex) *ppindex = pindex;
    return true;
}
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits)) {
        ...
        error("CheckBlockHeader() : proof of work failed for block %s, nBits %08x", block.GetHash().ToString(), block.nBits);
        // Set state and return false
...
// --- InitBlockIndex (Modified for EVM - With fix for double initialization) ---
// main.cpp
// main.cpp

// ... other includes ...
#include "txdb.h" // For pcoinsTip, CCoinsViewCache
// ...

bool InitBlockIndex() {
    LOCK(cs_main);
    CValidationState state; // Declare state here to be usable by AbortNode

    // Check if already initialized (e.g., block index loaded but EVM state needs init on reindex)
    // Genesis check is still relevant but moved slightly later
    // if (chainActive.Genesis() != nullptr) return true; // This might be too early if reindexing

    LogPrintf("Initializing databases...\n");
    // ... (Read/set flags fTxIndex, fLogEvents, fAddressIndex from pblocktree if necessary) ...
    // Example: if(pblocktree) pblocktree->ReadFlag("txindex", fTxIndex); etc.

    // --- EVM Init ---
    // ---> ADD CHECK: Only initialize EVM state if it hasn't been done yet <---
    if (globalState) {
        LogPrint("evm", "InitBlockIndex: EVM globalState already initialized, skipping.\n");
    } else {
        // ---> WRAP existing EVM init logic in the else block <---
        LogPrint("evm", "InitBlockIndex: Initializing EVM databases...\n");
        fs::path statePath = GetDataDir(false) / "stateQtum";
        fs::path utxoPath = GetDataDir(false) / "stateQtumUTXO";
        fs::path resultsPath = GetDataDir(false) / "results"; // Path for storage results

        // ---> USE STANDARD EMPTY HASHES <---
        dev::h256 stateRoot = dev::sha3(dev::rlp(""));
        dev::h256 utxoRoot = dev::sha3(dev::rlp(""));

        try {
            // Ensure parent directories exist BEFORE opening databases
             fs::create_directories(statePath.parent_path());
             fs::create_directories(utxoPath.parent_path());
             // StorageResults constructor now handles 'results' dir creation check

            // Initialize QtumState
            globalState = std::unique_ptr<QtumState>(new QtumState(
                dev::u256(0), // Initial difficulty, adjust if needed
                QtumState::openDB(statePath.string(), stateRoot, dev::WithExisting::Trust), // Use Trust on initial open
                utxoPath.string()
            ));
            // Set in-memory roots after creation
            globalState->setRoot(stateRoot);
            globalState->setRootUTXO(utxoRoot);

            // Initialize StorageResults (pass parent path)
            pstorageresult = std::unique_ptr<StorageResults>(new StorageResults(resultsPath.parent_path().string()));

            LogPrintf("Initialized EVM state database at %s\n", statePath.string());
            LogPrintf("Using initial state root: %s\n", stateRoot.hex());
            LogPrintf("Using initial UTXO root: %s\n", utxoRoot.hex());
            LogPrintf("Initialized StorageResults database in %s\n", resultsPath.string());

        } catch (const std::exception& e) {
            // Clear pointers on failure to avoid inconsistent state
            globalState.reset();
            pstorageresult.reset();
            strMiscWarning = _("Error initializing EVM database") + std::string(": ") + e.what();
            return error("Failed to initialize EVM database (state: %s, utxo: %s, results: %s): %s. You may need to reindex or delete stateQtum/stateQtumUTXO/results directories.", statePath.string(), utxoPath.string(), resultsPath.string(), e.what());
        }
    } // ---> END OF else block for EVM init <---
    // --- End EVM Init ---


    // Check if Genesis block already exists in the index (e.g., after loading index but before reindex finishes)
    // If reindexing, genesis might not be in chainActive yet, but could be in mapBlockIndex
    BlockMap::iterator mi = mapBlockIndex.find(Params().GetConsensus().hashGenesisBlock);
    if (mi != mapBlockIndex.end() && (mi->second->nStatus & BLOCK_HAVE_DATA)) {
         // Genesis block already processed and written, ensure chainActive points to it if needed
         if (!chainActive.Genesis()) {
              LogPrintf("InitBlockIndex: Genesis block found in index, setting chainActive tip.\n");
              chainActive.SetTip(mi->second);
              // May need to PruneBlockIndexCandidates() here if called during reindex startup
              // PruneBlockIndexCandidates();
         }
         LogPrintf("InitBlockIndex: Genesis block already present.\n");
         return true; // Already initialized
    }


    // Genesis block handling (Only if not already present/processed, e.g., first run or after wipe during reindex)
    LogPrintf("InitBlockIndex: Processing Genesis Block...\n");
    try {
        CBlock& block = const_cast<CBlock&>(Params().GenesisBlock());

        // Check disk space before writing
        unsigned int nBlockSize = ::GetSerializeSize(block, SER_DISK, CLIENT_VERSION);
        if (!CheckDiskSpace(nBlockSize + 8)) // Estimate space needed
             return error("InitBlockIndex() : Not enough disk space for genesis block");

        CDiskBlockPos blockPos;
        // CValidationState state; // Already declared at the beginning of the function

        // Find position and write genesis block to blk00000.dat
        if (!FindBlockPos(state, blockPos, nBlockSize + 8, 0, block.GetBlockTime(), false))
             return error("InitBlockIndex() : FindBlockPos failed for genesis block: %s", state.GetRejectReason());
        if (!WriteBlockToDisk(block, blockPos))
             return AbortNode(state, "Failed to write genesis block"); // Use AbortNode on critical I/O failure

        // Add genesis block to the block index map
        CBlockIndex* pindex = AddToBlockIndex(block);
        if (!pindex)
             return error("InitBlockIndex() : AddToBlockIndex failed for genesis block");

        // ---> SET CORRECT GENESIS ROOTS IN INDEX <---
        // Retrieve the roots used/set during EVM initialization above
        dev::h256 stateRoot = dev::sha3(dev::rlp("")); // Recalculate or get from globalState if needed
        dev::h256 utxoRoot = dev::sha3(dev::rlp(""));
        pindex->hashStateRoot = h256Touint(stateRoot);
        pindex->hashUTXORoot = h256Touint(utxoRoot);
        pindex->nStatus |= BLOCK_HAVE_STATE; // Mark state as present for genesis

        setDirtyBlockIndex.insert(pindex);

        // Mark genesis as having data and process potential descendants (should be none)
        if (!ReceivedBlockTransactions(block, state, pindex, blockPos))
             return error("InitBlockIndex() : ReceivedBlockTransactions failed for genesis block: %s", state.GetRejectReason());

        // Set genesis as the current chain tip
        chainActive.SetTip(pindex);
        // Prune candidates as the tip is now genesis
        PruneBlockIndexCandidates();

        // ******** ADD THIS SECTION (FIX for "Previous block mismatch") ********
        // Explicitly set the best block for pcoinsTip to genesis
        // This ensures that when FlushStateToDisk calls pcoinsTip->Flush(),
        // the correct best block (genesis) is written to the underlying coin DB.
        if (pcoinsTip) {
            pcoinsTip->SetBestBlock(pindex->GetBlockHash());
            LogPrint("init", "InitBlockIndex: Set pcoinsTip best block to genesis: %s\n", pindex->GetBlockHash().ToString());
        } else {
            // This should ideally not happen if initialization order is correct
            return AbortNode(state, "pcoinsTip is null before genesis flush in InitBlockIndex");
        }
        // **********************************************************************

        // Flush everything to disk to ensure genesis state is saved
        if (!FlushStateToDisk(state, FlushStateMode::ALWAYS))
            return false; // Error handled by FlushStateToDisk

        LogPrintf("InitBlockIndex: Genesis block successfully processed and written.\n");

    } catch (const std::runtime_error& e) {
        return error("InitBlockIndex() : runtime error during genesis processing: %s", e.what());
    } catch (const std::exception& e) {
        return error("InitBlockIndex() : exception during genesis processing: %s", e.what());
    }
...
// ---> Ensure INCLUDES are present at the top <---
#include <qtum/qtumstate.h>
#include <qtum/storageresults.h>
#include <qtum/qtumutils.h>
#include <libdevcore/CommonData.h>
#include <libdevcore/RLP.h>
#include <util/convert.h>
#include <libethereum/State.h> // Needed for CacheState::Flushed
#include <libethereum/ChainParams.h>
#include <libethereum/LastBlockHashesFace.h>
#include <libethereum/TransactionReceipt.h>
#include <libethereum/Executive.h>
...
// --- ConnectBlock Function (v4 - Attempting all fixes) ---
// Replace the existing ConnectBlock function entirely with this one.
...
LogPrint("validation", "ConnectBlock start: hash=%s height=%d fJustCheck=%d\n",
hashBlock.ToString().substr(0,10), pindex->nHeight, fJustCheck);

// Timer variables
int64_t nTimeStartConnectBlock = GetTimeMicros(); // Overall timer
int64_t nTimeCheck = 0, nTimeConnectLoop = 0, nTimeExec = 0, nTimeProcess = 0, nTimeVal = 0, nTimeCommit = 0, nTimeWrite = 0, nTimeCallbacks = 0; // Separate timers for sections
int64_t nTimeStartSection;

nTimeStartSection = GetTimeMicros();
// Perform context-independent block checks
if (!fAlreadyChecked && !CheckBlock(block, state, !fJustCheck, !fJustCheck, !fJustCheck)) {
LogPrintf("ERROR: ConnectBlock: CheckBlock failed for %s: %s\n", hashBlock.ToString(), state.GetRejectReason());
if (state.CorruptionPossible()) {
return AbortNode(state, "Corrupt block found indicating potential hardware failure; shutting down");
}
return false;
}
nTimeCheck = GetTimeMicros() - nTimeStartSection;

// Verify view corresponds to previous block
uint256 hashPrevBlock = pindex->pprev == nullptr ? consensus.hashGenesisBlock : pindex->pprev->GetBlockHash();
if (hashPrevBlock != view.GetBestBlock()) {
LogPrintf("%s: ERROR - Previous block mismatch. pindex->pprev->GetBlockHash()=%s, view.GetBestBlock()=%s\n",
    __func__, hashPrevBlock.ToString(), view.GetBestBlock().ToString());
return AbortNode(state, "Critical failure: Previous block mismatch in ConnectBlock");
}

// ---> EVM/State Root Logic: Backup state and Set Previous State <---
dev::h256 oldStateRoot, oldUTXORoot;
bool stateRootsActive = (pindex->nHeight >= consensus.height_start_StateRoots);
bool stateLoaded = false;
bool revertEVMStateOnError = false;
dev::h256 calculatedStateRoot, calculatedUtxoRoot;

if (stateRootsActive) {
if (!globalState) { return AbortNode(state, "Critical failure: State roots active but globalState is null in ConnectBlock"); }
oldStateRoot = globalState->rootHash(); oldUTXORoot = globalState->rootHashUTXO();
LogPrint("evm", "ConnectBlock: Height %d >= %d. State roots active. Backing up roots: State=%s UTXO=%s\n", pindex->nHeight, consensus.height_start_StateRoots, oldStateRoot.hex(), oldUTXORoot.hex());
revertEVMStateOnError = true;

if (pindex->pprev) {
if (!(pindex->pprev->nStatus & BLOCK_HAVE_STATE) && pindex->pprev->nHeight > 0) { return AbortNode(state, strprintf("ConnectBlock: Previous block index (H:%d %s) missing state roots after fork height.", pindex->pprev->nHeight, pindex->pprev->GetBlockHash().ToString())); }
dev::h256 prevStateRoot = uintToh256(pindex->pprev->hashStateRoot); dev::h256 prevUtxoRoot = uintToh256(pindex->pprev->hashUTXORoot);
try {
   globalState->setRoot(prevStateRoot); globalState->setRootUTXO(prevUtxoRoot); stateLoaded = true;
   LogPrint("evm", "ConnectBlock: Set state to previous block (H:%d %s) roots: State=%s UTXO=%s\n", pindex->pprev->nHeight, pindex->pprev->GetBlockHash().ToString(), prevStateRoot.hex(), prevUtxoRoot.hex());
} catch (const std::exception& e) { return AbortNode(state, strprintf("ConnectBlock: Failed to set state roots for block %d from previous %d: %s", pindex->nHeight, pindex->pprev->nHeight, e.what())); }
} else {
if(hashBlock != consensus.hashGenesisBlock) { return AbortNode(state, "Critical failure: pindex->pprev is null for non-genesis block in ConnectBlock"); }
dev::h256 expectedEmptyRoot = dev::sha3(dev::rlp(""));
if(globalState->rootHash() != expectedEmptyRoot || globalState->rootHashUTXO() != expectedEmptyRoot) {
   LogPrintf("WARNING: ConnectBlock: Genesis predecessor is null, but globalState roots are not empty! Resetting.\n");
   globalState->setRoot(expectedEmptyRoot); globalState->setRootUTXO(expectedEmptyRoot);
}
stateLoaded = true;
}
}
// ---> END EVM/State Root Preparation <---


// Special case for the genesis block
if (hashBlock == consensus.hashGenesisBlock) {
LogPrint("validation", "ConnectBlock: Processing Genesis Block %s\n", hashBlock.ToString());
if (!fJustCheck) {
if (!pindex) { return AbortNode(state, "ConnectBlock: Genesis block index pointer is null."); }
dev::h256 expectedStateRoot = dev::sha3(dev::rlp("")); dev::h256 expectedUtxoRoot = dev::sha3(dev::rlp(""));
pindex->hashStateRoot = h256Touint(expectedStateRoot); pindex->hashUTXORoot = h256Touint(expectedUtxoRoot);
pindex->nVersion = block.nVersion; pindex->nStatus |= BLOCK_HAVE_STATE;
setDirtyBlockIndex.insert(pindex);
view.SetBestBlock(pindex->GetBlockHash());

if (globalState) {
  try {
      if(globalState->rootHash() != expectedStateRoot || globalState->rootHashUTXO() != expectedUtxoRoot) { globalState->setRoot(expectedStateRoot); globalState->setRootUTXO(expectedUtxoRoot); }
      globalState->db().commit(); // Try commit without CacheState
      globalState->dbUtxo().commit(); // Try commit without CacheState
      LogPrint("evm", "ConnectBlock: Committed initial empty EVM state for genesis.\n");
  } catch (const std::exception& e) { return AbortNode(state, "Failed to commit initial EVM state for genesis: " + std::string(e.what())); }
} else if (stateRootsActive) { return AbortNode(state, "Critical failure: globalState is null during Genesis connection when state roots are active."); }
LogPrintf("ConnectBlock: Successfully connected Genesis Block %s\n", pindex->GetBlockHash().ToString());
}
return true;
}

// --- Regular block connection ---
const int last_pow_block = consensus.height_last_PoW;
if (pindex->nHeight <= last_pow_block && block.IsProofOfStake()) return state.DoS(100, error("ConnectBlock() : PoS period not active"), REJECT_INVALID, "PoS-early");
if (pindex->nHeight > last_pow_block && block.IsProofOfWork()) return state.DoS(100, error("ConnectBlock() : PoW period ended"), REJECT_INVALID, "PoW-ended");

bool fScriptChecks = pindex->nHeight >= Checkpoints::GetTotalBlocksEstimate();
bool fCLTVIsActivated = fScriptChecks && pindex->pprev && (pindex->pprev->nHeight >= consensus.height_start_BIP65);
CCheckQueueControl<CScriptCheck> control(fScriptChecks && nScriptCheckThreads ? &scriptcheckqueue : nullptr);

CAmount nFees = 0; CAmount nUnspendable = 0; int nInputs = 0; unsigned int nSigOps = 0;
unsigned int nMaxBlockSigOps = MAX_BLOCK_SIGOPS_CURRENT;
CDiskTxPos pos(pindex->GetBlockPos(), GetSizeOfCompactSize(block.vtx.size()));
std::vector<std::pair<uint256, CDiskTxPos>> vPos; vPos.reserve(block.vtx.size());
CBlockUndo blockUndo; blockUndo.vtxundo.reserve(block.vtx.size() - 1);
CAmount nValueOut = 0; CAmount nValueIn = 0;

ExtractQtumTX qtumTxConvert;
std::vector<CTransactionRef> blockTxRefs; blockTxRefs.reserve(block.vtx.size());
for(const auto& tx : block.vtx) { blockTxRefs.push_back(MakeTransactionRef(tx)); }
unsigned int contractFlags = GetContractScriptFlags(pindex->nHeight, consensus);
std::vector<QtumTransaction> qtumTransactions;

int64_t nTimeLoopStart = GetTimeMicros(); // Correct timer start point

// Process Transactions (Main Loop)
for (unsigned int i = 0; i < block.vtx.size(); i++) {
const CTransaction& tx = block.vtx[i]; const uint256 txhash = tx.GetHash();

const CCoins* coins = view.AccessCoins(txhash);
if (coins && !coins->IsPruned()) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : tried to overwrite transaction %s", txhash.ToString()), REJECT_INVALID, "bad-txns-BIP30"); }
nInputs += tx.vin.size(); nSigOps += GetLegacySigOpCount(tx);
if (nSigOps > nMaxBlockSigOps) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : too many legacy sigops in block %s", hashBlock.ToString()), REJECT_INVALID, "bad-blk-sigops"); }
// Obsolete ZC checks...
if (!IsInitialBlockDownload() && tx.ContainsZerocoins()) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : Zerocoin transactions have been permanently disabled")); }

// EVM Extraction
if (globalState && (TxHasCreateOrCall(tx) || TxHasOpSender(tx))) {
QtumTxConverter converter(tx, &mempool, &view, &blockTxRefs, contractFlags);
if (converter.extractionQtumTransactions(qtumTxConvert)) { qtumTransactions.insert(qtumTransactions.end(), qtumTxConvert.first.begin(), qtumTxConvert.first.end()); LogPrint("evm", "ConnectBlock: Extracted %d EVM txs from Bitcoin tx %s\n", qtumTxConvert.first.size(), txhash.ToString()); }
else { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock: Failed to parse EVM transaction for tx %s", txhash.ToString()), REJECT_INVALID, "bad-tx-evm-parse"); }
}
// Standard Input/Output Processing
if (!tx.IsCoinBase() && !tx.HasZerocoinSpendInputs()) {
if (!view.HaveInputs(tx)) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : inputs missing/spent for tx %s", txhash.ToString()), REJECT_INVALID, "bad-txns-inputs-missingorspent"); }
nSigOps += GetP2SHSigOpCount(tx, view);
if (nSigOps > nMaxBlockSigOps) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : too many total sigops after P2SH in block %s", hashBlock.ToString()), REJECT_INVALID, "bad-blk-sigops"); }
CAmount txValueIn = view.GetValueIn(tx); nValueIn += txValueIn;
if (!tx.IsCoinStake()) {
  CAmount txValueOut = tx.GetValueOut(); if (txValueIn < txValueOut) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : %s value in (%s) < value out (%s)", txhash.ToString(), FormatMoney(txValueIn), FormatMoney(txValueOut)), REJECT_INVALID, "bad-txns-in-belowout"); }
  CAmount nTxFee = txValueIn - txValueOut; if (!consensus.MoneyRange(nTxFee)) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : %s nTxFee out of range (%s)", txhash.ToString(), FormatMoney(nTxFee)), REJECT_INVALID, "bad-txns-fee-outofrange"); }
  nFees += nTxFee; if (!consensus.MoneyRange(nFees)) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : cumulative block nFees out of range (%s)", FormatMoney(nFees)), REJECT_INVALID, "bad-blk-accumulated-fee-outofrange"); }
}
std::vector<CScriptCheck> vChecks; unsigned int flags = SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG; if (fCLTVIsActivated) flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;
if (!CheckInputs(tx, state, view, fScriptChecks, flags, false, nScriptCheckThreads ? &vChecks : nullptr)) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } LogPrintf("ERROR: ConnectBlock: CheckInputs failed for tx %s: %s\n", txhash.ToString(), state.GetRejectReason()); return false; }
control.Add(vChecks);
if (!fJustCheck) { for (const CTxOut& txOut : tx.vout) { if (txOut.scriptPubKey.IsUnspendable()) nUnspendable += txOut.nValue; } }
}
nValueOut += tx.GetValueOut();
CTxUndo undoDummy; bool needsUndo = (i > 0 && !tx.IsCoinBase() && !tx.IsCoinStake() && !tx.HasZerocoinSpendInputs());
if (needsUndo) { blockUndo.vtxundo.emplace_back(); UpdateCoins(tx, state, view, blockUndo.vtxundo.back(), pindex->nHeight); }
else { UpdateCoins(tx, state, view, undoDummy, pindex->nHeight); }
vPos.emplace_back(txhash, pos); pos.nTxOffset += ::GetSerializeSize(tx, SER_DISK, CLIENT_VERSION);
} // End Transaction Loop
nTimeConnectLoop = GetTimeMicros() - nTimeLoopStart;

nTimeStartSection = GetTimeMicros(); // Timer for EVM execution
// ---> EVM Logic: Execute EVM Transactions <---
std::vector<ResultExecute> execResults; ByteCodeExecResult bceResult; bool evm_exec_failed = false;
if (globalState && !qtumTransactions.empty()) {
LogPrint("evm", "ConnectBlock: Executing %d EVM transactions for block %d (Hash: %s)\n", qtumTransactions.size(), pindex->nHeight, hashBlock.ToString());
uint64_t blockGasLimit = 40000000; // TODO: Fetch dynamic gas limit
ByteCodeExec exec(block, qtumTransactions, blockGasLimit, pindex->pprev);
if (exec.performByteCode(dev::eth::Permanence::Committed)) {
execResults = exec.getResult();
if (!exec.processingResults(bceResult)) {
   LogPrintf("ERROR: ConnectBlock: Failed processing EVM execution results for block %s\n", hashBlock.ToString()); state.DoS(100, false, REJECT_INVALID, "bad-blk-evm-results"); evm_exec_failed = true;
} else {
   for (const CTransaction& vtx : bceResult.valueTransfers) { LogPrint("evm", "ConnectBlock: Applying EVM value transfer tx %s\n", vtx.GetHash().ToString()); CTxUndo dummyUndo; UpdateCoins(vtx, state, view, dummyUndo, pindex->nHeight); nValueOut += vtx.GetValueOut(); }
   if (bceResult.refundSender > 0) { LogPrint("evm", "ConnectBlock: Applying EVM gas refund of %s\n", FormatMoney(bceResult.refundSender)); CMutableTransaction refundTx; refundTx.vin.push_back(CTxIn(hashBlock, (uint32_t)-1)); refundTx.vout = bceResult.refundOutputs; CTxUndo dummyUndo; UpdateCoins(CTransaction(refundTx), state, view, dummyUndo, pindex->nHeight); nValueOut += bceResult.refundSender; }
   if (!fJustCheck && fRecordLogOpcodes && pstorageresult) {
       for(size_t k=0; k<execResults.size(); ++k){
           const QtumTransaction& qtx = qtumTransactions[k]; dev::h256 qtxHash = qtx.getHashWith();
           const QtumTransactionReceipt& receipt = execResults[k].txRec; const dev::eth::ExecutionResult& evmResult = execResults[k].execRes;
           TransactionReceiptInfo receiptInfo;
           receiptInfo.blockHash = pindex->GetBlockHash(); receiptInfo.blockNumber = pindex->nHeight;
           receiptInfo.transactionHash = h256Touint(qtxHash);
           receiptInfo.from = qtx.sender(); receiptInfo.to = qtx.isCreation() ? dev::Address() : qtx.receiveAddress();
           receiptInfo.cumulativeGasUsed = static_cast<uint64_t>(receipt.cumulativeGasUsed()); receiptInfo.gasUsed = static_cast<uint64_t>(evmResult.gasUsed);
           receiptInfo.contractAddress = evmResult.newAddress; receiptInfo.logs = receipt.log();
           receiptInfo.excepted = evmResult.excepted; receiptInfo.bloom = receipt.bloom();
           receiptInfo.stateRoot = receipt.stateRoot(); receiptInfo.utxoRoot = receipt.utxoRoot();
           std::vector<TransactionReceiptInfo> receiptInfoVec = {receiptInfo};
           pstorageresult->addResult(qtxHash, receiptInfoVec);
       }
       writeVMlog(execResults, CTransaction(), block);
   }
}
} else { LogPrintf("ERROR: ConnectBlock: EVM bytecode execution failed catastrophically for block %s\n", hashBlock.ToString()); state.DoS(100, false, REJECT_INVALID, "bad-blk-evm-exec"); evm_exec_failed = true; }
if (evm_exec_failed && revertEVMStateOnError) {
LogPrint("evm", "ConnectBlock: Reverting EVM state due to execution failure.\n");
try { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } catch(const std::exception& e) { return AbortNode(state, strprintf("ConnectBlock: Failed to revert EVM state after error: %s", e.what())); }
}
} // End EVM execution block
if(evm_exec_failed) return false;
nTimeExec = GetTimeMicros() - nTimeStartSection;
// ---> END EVM Execution <---

nTimeStartSection = GetTimeMicros(); // Timer for post-EVM checks
// ... (UpdateZWAGESupply - review) ...
...
// Block Mint/Reward Validation
CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight);
if (block.IsProofOfWork()) { nExpectedMint += nFees; } else if (!fJustCheck) { nUnspendable += nFees; }
if (!IsBlockValueValid(block, nExpectedMint, nMint)) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock() : reward pays too much (actual=%s vs limit=%s)", FormatMoney(nMint), FormatMoney(nExpectedMint)), REJECT_INVALID, "bad-cb-amount"); }

// Wait for script verification
if (!control.Wait()) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("%s: Script CheckQueue failed", __func__), REJECT_INVALID, "block-validation-failed"); }
nTimeProcess = GetTimeMicros() - nTimeStartSection;

nTimeStartSection = GetTimeMicros(); // Timer for State Root Validation
// ---> EVM Logic: Validate State Roots <---
bool stateRootsValid = false;
if (stateRootsActive) {
if(!globalState) return AbortNode(state, "Critical failure: globalState null before state root validation.");
calculatedStateRoot = globalState->rootHash(); calculatedUtxoRoot = globalState->rootHashUTXO();
uint256 headerStateRoot = block.hashStateRoot; uint256 headerUtxoRoot = block.hashUTXORoot;
LogPrint("evm", "ConnectBlock: Validating roots for block %d (Hash: %s)\n", pindex->nHeight, hashBlock.ToString());
LogPrint("evm", "  Block Header State Root: %s\n", headerStateRoot.ToString()); LogPrint("evm", "  Calculated State Root: %s\n", h256Touint(calculatedStateRoot).ToString());
LogPrint("evm", "  Block Header UTXO Root:  %s\n", headerUtxoRoot.ToString()); LogPrint("evm", "  Calculated UTXO Root:  %s\n", h256Touint(calculatedUtxoRoot).ToString());
if (headerStateRoot != h256Touint(calculatedStateRoot)) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock(): State root hash mismatch (Header: %s, Calculated: %s)", headerStateRoot.ToString(), h256Touint(calculatedStateRoot).ToString()), REJECT_INVALID, "bad-txns-stateRoot-mismatch"); }
if (headerUtxoRoot != h256Touint(calculatedUtxoRoot)) { if (revertEVMStateOnError) { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } return state.DoS(100, error("ConnectBlock(): UTXO root hash mismatch (Header: %s, Calculated: %s)", headerUtxoRoot.ToString(), h256Touint(calculatedUtxoRoot).ToString()), REJECT_INVALID, "bad-txns-utxoRoot-mismatch"); }
stateRootsValid = true; LogPrint("evm", "ConnectBlock: State roots verified successfully for block %d\n", pindex->nHeight);
} else { stateRootsValid = true; }
nTimeVal = GetTimeMicros() - nTimeStartSection; // End timer for State Root Validation
// ---> END EVM State Root Validation <---


// Log benchmark breakdown
LogPrint("bench", "      - CheckBlock: %.2fms\n", 0.001 * nTimeCheck);
LogPrint("bench", "      - ConnectLoop+UTXO: %.2fms\n", 0.001 * nTimeConnectLoop);
LogPrint("bench", "    - EVMExec: %.2fms\n", 0.001 * nTimeExec);
LogPrint("bench", "    - ScriptVerify+PostProc: %.2fms\n", 0.001 * nTimeProcess); // Renamed for clarity
LogPrint("bench", "    - RootValidate: %.2fms\n", 0.001 * nTimeVal);

// If fJustCheck, revert and return true
if (fJustCheck) {
LogPrint("validation", "ConnectBlock: fJustCheck is true, returning without commit/write.\n");
if (revertEVMStateOnError) { try { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } catch (const std::exception& e) { return AbortNode(state, strprintf("ConnectBlock: Failed to revert EVM state during fJustCheck: %s", e.what())); } }
return true;
}

// --- Persist changes to disk/index ---
nTimeStartSection = GetTimeMicros(); // Timer for EVM Commit
// ---> EVM Logic: Commit State Changes <---
if (stateRootsActive && globalState) {
try {
globalState->db().commit(); // Try commit without CacheState
globalState->dbUtxo().commit(); // Try commit without CacheState
LogPrint("evm", "ConnectBlock: Committed EVM state for block %d\n", pindex->nHeight);
if(pstorageresult) pstorageresult->commitResults();
} catch (const std::exception& e) {
try { globalState->setRoot(oldStateRoot); globalState->setRootUTXO(oldUTXORoot); } catch(...) {}
return AbortNode(state, "Failed to commit EVM state changes: " + std::string(e.what()));
}
}
nTimeCommit = GetTimeMicros() - nTimeStartSection;
// ---> END EVM State Commit <---

nTimeStartSection = GetTimeMicros(); // Timer for Disk Writes
// Update burned supply
nBurnedCoins += nUnspendable;
if (!IsInitialBlockDownload() || pindex->nHeight % 1000 == 0) { if (pblocktree) pblocktree->WriteInt("burned", std::round(nBurnedCoins / COIN)); }

// Write undo information
if (pindex->GetUndoPos().IsNull() || !pindex->IsValid(BLOCK_VALID_SCRIPTS)) {
if (pindex->GetUndoPos().IsNull()) {
CDiskBlockPos diskPosUndo; unsigned int undoSize = ::GetSerializeSize(blockUndo, SER_DISK, CLIENT_VERSION);
if (!pindex->pprev) return AbortNode(state, "ConnectBlock: pprev is null when writing undo data for non-genesis block.");
if (!FindUndoPos(state, pindex->nFile, diskPosUndo, undoSize + 40)) return error("ConnectBlock() : FindUndoPos failed");
if (!blockUndo.WriteToDisk(diskPosUndo, pindex->pprev->GetBlockHash())) return AbortNode(state, "Failed to write undo data");
pindex->nUndoPos = diskPosUndo.nPos; pindex->nStatus |= BLOCK_HAVE_UNDO;
}
pindex->RaiseValidity(BLOCK_VALID_SCRIPTS); setDirtyBlockIndex.insert(pindex);
}

// Store roots and size in index
if (stateRootsActive && stateRootsValid) {
pindex->hashStateRoot = h256Touint(calculatedStateRoot); pindex->hashUTXORoot = h256Touint(calculatedUtxoRoot);
pindex->nStatus |= BLOCK_HAVE_STATE;
LogPrint("db", "ConnectBlock: Stored state roots in pindex %d: State=%s UTXO=%s\n", pindex->nHeight, pindex->hashStateRoot.ToString(), pindex->hashUTXORoot.ToString());
}
pindex->nBlockSize = ::GetSerializeSize(block, SER_DISK, CLIENT_VERSION);
setDirtyBlockIndex.insert(pindex); // Mark dirty to save roots/size/status

// Write Tx Index
if (fTxIndex) { if (!pblocktree || !pblocktree->WriteTxIndex(vPos)) return AbortNode(state, "Failed to write transaction index"); }

// Update UTXO view tip
view.SetBestBlock(pindex->GetBlockHash());

nTimeWrite = GetTimeMicros() - nTimeStartSection;

// Log final benchmark numbers
LogPrint("bench", "    - EVMCommit: %.2fms\n", 0.001 * nTimeCommit);
LogPrint("bench", "    - DiskWrite: %.2fms\n", 0.001 * nTimeWrite);

// Wallet signals/callbacks
nTimeStartSection = GetTimeMicros();
static uint256 hashPrevBestCoinBase;
GetMainSignals().UpdatedTransaction(hashPrevBestCoinBase);
hashPrevBestCoinBase = block.vtx[0].GetHash();
nTimeCallbacks = GetTimeMicros() - nTimeStartSection;
LogPrint("bench", "    - Callbacks: %.2fms [%.2fs]\n", 0.001 * nTimeCallbacks, nTimeCallbacks * 0.000001);

// ... (Obsolete ZC DB checksum update/wipe) ...

LogPrintf("ConnectBlock: Successfully connected block %s (Height: %d)\n", hashBlock.ToString(), pindex->nHeight);
return true;
}


// --- DisconnectBlock Function (Corrected Version v2) ---
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
AssertLockHeld(cs_main);
uint256 blockHashToDisconnect = pindex->GetBlockHash();
LogPrint("validation", "DisconnectBlock: Disconnecting block %s (Height: %d)\n", blockHashToDisconnect.ToString(), pindex->nHeight);

if (pindex->GetBlockHash() != view.GetBestBlock()) {
LogPrintf("%s: ERROR - Block to disconnect (%s) is not current view tip (%s)\n", __func__, pindex->GetBlockHash().ToString(), view.GetBestBlock().ToString());
return error("DisconnectBlock() : block %s is not the current view best block %s", pindex->GetBlockHash().ToString(), view.GetBestBlock().ToString());
}

if (pfClean) *pfClean = false;
bool fClean = true;

// ---> EVM/State Root Logic: Prepare for State Reversal <---
const Consensus::Params& consensus = Params().GetConsensus();
bool stateRootsActive = (pindex->nHeight >= consensus.height_start_StateRoots);
dev::h256 parentStateRoot, parentUtxoRoot;
bool revertState = false;

if (stateRootsActive) {
if (!globalState) { return AbortNode(state, "Critical failure: State roots active but globalState is null in DisconnectBlock"); }
if (!pindex->pprev) { return AbortNode(state, "Critical failure: Cannot disconnect block with null pprev when state roots active"); }
if (!(pindex->pprev->nStatus & BLOCK_HAVE_STATE) && pindex->pprev->nHeight > 0) { return AbortNode(state, strprintf("DisconnectBlock: Parent block index (H:%d %s) missing state roots during disconnect.", pindex->pprev->nHeight, pindex->pprev->GetBlockHash().ToString())); }
parentStateRoot = uintToh256(pindex->pprev->hashStateRoot); parentUtxoRoot = uintToh256(pindex->pprev->hashUTXORoot);
revertState = true;
LogPrint("evm", "DisconnectBlock: Preparing to revert state for block %d (Hash: %s) to parent (H:%d) roots: State=%s UTXO=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString(), pindex->pprev->nHeight, parentStateRoot.hex(), parentUtxoRoot.hex());
}
// ---> END EVM Preparation <---

// Load undo information
CBlockUndo blockUndo; // Correct variable name
CDiskBlockPos pos = pindex->GetUndoPos();
if (pos.IsNull()) { return error("DisconnectBlock() : no undo data available for block %s", pindex->GetBlockHash().ToString()); }
if (!pindex->pprev) return AbortNode(state, "DisconnectBlock: pprev is null when reading undo data for non-genesis block.");
if (!blockUndo.ReadFromDisk(pos, pindex->pprev->GetBlockHash())) { return error("DisconnectBlock() : failure reading undo data for block %s", pindex->GetBlockHash().ToString()); }
if (blockUndo.vtxundo.size() + 1 != block.vtx.size()) { return error("DisconnectBlock() : block and undo data inconsistent for block %s (vtx=%zu, vtxundo=%zu)", pindex->GetBlockHash().ToString(), block.vtx.size(), blockUndo.vtxundo.size()); }

// --- Process transactions in REVERSE order ---
for (int i = block.vtx.size() - 1; i >= 0; i--) {
const CTransaction& tx = block.vtx[i]; uint256 hash = tx.GetHash();
// ... (Undo Zerocoin DB operations) ...
// --- Standard UTXO Undo ---
{ CCoinsModifier outs = view.ModifyCoins(hash); if (outs->IsPruned()) { fClean = fClean && error("DisconnectBlock(): Output %s not found in UTXO set (already pruned?)", hash.ToString()); } outs->Clear(); }
if (!tx.IsCoinBase() && !tx.HasZerocoinSpendInputs()) {
if (i == 0 || (unsigned)(i - 1) >= blockUndo.vtxundo.size()) { return error("DisconnectBlock() : undo data index out of bounds for tx index %d", i); } // Fixed typo
const CTxUndo& txundo = blockUndo.vtxundo[i - 1]; // Fixed typo
if (txundo.vprevout.size() != tx.vin.size()) { return error("DisconnectBlock() : transaction %s and undo data inconsistent (vin=%zu, vprevout=%zu)", tx.GetHash().ToString(), tx.vin.size(), txundo.vprevout.size()); }
for (unsigned int j = tx.vin.size(); j-- > 0;) {
  const COutPoint& outpoint = tx.vin[j].prevout; const CTxInUndo& undo = txundo.vprevout[j]; CCoinsModifier coins = view.ModifyCoins(outpoint.hash);
  if (undo.nHeight != 0) { if (!coins->IsPruned()) { fClean = fClean && error("DisconnectBlock() : undo data overwriting existing transaction %s", outpoint.hash.ToString()); } coins->Clear(); coins->fCoinBase = undo.fCoinBase; coins->nHeight = undo.nHeight; coins->nVersion = undo.nVersion; }
  else { if (coins->IsPruned()) { fClean = fClean && error("DisconnectBlock() : undo data adding output to missing transaction %s", outpoint.hash.ToString()); } }
  if (coins->IsAvailable(outpoint.n)) { fClean = fClean && error("DisconnectBlock() : undo data overwriting existing output %s:%d", outpoint.hash.ToString(), outpoint.n); }
  if (coins->vout.size() < outpoint.n + 1) { coins->vout.resize(outpoint.n + 1); } coins->vout[outpoint.n] = undo.txout;
}
}
// ---> EVM Logic: Delete storage results <---
if (pstorageresult){ // No fJustCheck here
std::vector<const CTransaction*> txsToDelete; txsToDelete.push_back(&tx);
pstorageresult->deleteResults(txsToDelete);
}
// ---> END EVM Storage Delete <---
} // End transaction loop

// Move best block pointer in UTXO view back to the parent
if (!pindex->pprev) return AbortNode(state, "DisconnectBlock: pprev is null when setting best block for non-genesis block.");
view.SetBestBlock(pindex->pprev->GetBlockHash());

// ---> EVM Logic: Revert State Tree Roots and Commit <---
if (revertState) {
try {
globalState->setRoot(parentStateRoot); globalState->setRootUTXO(parentUtxoRoot);
globalState->db().commit(); // Try commit without CacheState
globalState->dbUtxo().commit(); // Try commit without CacheState
LogPrint("evm", "DisconnectBlock: Reverted and committed EVM state for block %d to parent roots\n", pindex->nHeight);
if(pstorageresult) pstorageresult->clearCacheResult(); // Clear storage cache after successful revert
} catch (const std::exception& e) { return AbortNode(state, "Failed to revert EVM state during disconnect: " + std::string(e.what())); }
}
// ---> END EVM State Reversal <---

// ... (Undo Zerocoin Accumulator Checksum DB changes) ...

// Update block index status
pindex->nStatus &= ~(BLOCK_VALID_SCRIPTS | BLOCK_HAVE_STATE | BLOCK_FAILED_MASK); // Fixed flag clearing
setDirtyBlockIndex.insert(pindex);

// Set pfClean output flag
if (pfClean) { *pfClean = fClean; }

LogPrintf("DisconnectBlock: Successfully disconnected block %s (Height: %d)\n", blockHashToDisconnect.ToString(), pindex->nHeight);

return state.IsValid();
}


// Add this function definition to main.cpp (if it's missing)
bool DisconnectBlocks(int nBlocks)
{
    LOCK(cs_main); // Lock cs_main for chain operations

    CValidationState state; // State object to potentially catch errors from DisconnectTip

    LogPrintf("%s: Got command to replay %d blocks\n", __func__, nBlocks);
    // Loop to disconnect the specified number of blocks + 1 (to disconnect nBlocks *from* the tip)
    for (int i = 0; i <= nBlocks; i++) {
         // Check if the chain is empty or at genesis before trying to disconnect
         if (!chainActive.Tip() || chainActive.Tip() == chainActive.Genesis()) { // Added Genesis check
             LogPrintf("%s: Chain already at genesis or empty, cannot disconnect further.\n", __func__);
             break; // Stop if there are no more blocks to disconnect
         }
         // Call DisconnectTip, which handles reading the block, reverting state, and updating the tip
         if (!DisconnectTip(state)) {
             // If DisconnectTip fails, log the error and stop
             LogPrintf("%s: DisconnectTip failed at block height %d: %s\n",
                       __func__, chainActive.Height() + 1, state.GetRejectReason()); // Height + 1 is the block that failed to disconnect
             // FlushStateToDisk might be needed here depending on error handling policy
             // FlushStateToDisk(state, FlushStateMode::ALWAYS); // Example
             return false; // Indicate failure
         }
         // Check if shutdown requested during the loop
         if (ShutdownRequested()) {
             LogPrintf("%s: Shutdown requested during disconnect.\n", __func__);
             return false; // Stop processing on shutdown request
         }
    }

    // If the loop completed successfully (or stopped early due to empty chain)
    return true;
}
